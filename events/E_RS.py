...
class E_RS(BasicFB):
    def __init__(self, name, position=None):
        super().__init__(name, "E_RS", "Event-driven bistable", Position=position)
        ...
        # ECC: START->SET(Condition=S), SET->RESET(Condition=R), RESET->SET(Condition=S)。
        # SET 态下 S 被忽略、RESET/START 态下 R 被忽略 => EO 只在 Q 真正改变时发出。
